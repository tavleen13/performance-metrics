...
def fetch_data(request):
    ...
    # Validate the 'SELECT' fields by converting to dictionary object and passing through SelectQueryRequestSerialiser
    ...
    # Validation on 'WHERE' query by converting to dictionary object and checking the DataType of the fields through FilterQueryRequestSerialiser
    ...
    # 'Group By' validation if given else pass empty list
    if group_by is not None and group_by != '':
        group_query_objects = group_by.split(',')
    else:
        group_query_objects = []

    # 'Order By' setting string as per the sort order
    ...
    # A common model method to handle all the use-cases, hence making it more generic for any new UseCase making it
    # more scalable
